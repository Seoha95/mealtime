...
public class TodayMenu extends JFrame {
	...
	public TodayMenu() throws ClassNotFoundException, SQLException {
		...
		int random_gagea_id;
		String curt = "";
		System.out.println(userinfo.getMu_jong() + "\t" + userinfo.getMu_rnn() + "\t" + userinfo.getMu_meaoon() + "\t"
				+ userinfo.getMu_ondo() + "\t" + userinfo.getMu_meat() + "\t" + userinfo.getMu_money() + "\t"
				+ userinfo.getG_jooso());

		
		ArrayList<Mt_VO> memuList = mtdao.getGageaList(userinfo.getMu_jong(), userinfo.getMu_rnn(),
				userinfo.getMu_meaoon(), userinfo.getMu_ondo(), userinfo.getMu_meat(), userinfo.getMu_money(),
				userinfo.getG_jooso());
		if (!memuList.isEmpty()) {
			...
			System.out
					.println(memuList.get(random_gagea_id).getG_id() + " " + memuList.get(random_gagea_id).getMu_id());
			...
			for (Mt_VO vo : gList) {
				System.out.print(vo.getG_name() + "\t" + vo.getG_jooso() + "\t" + vo.getG_phone() + "\t"
						+ vo.getMu_name() + "\t" + vo.getMu_money() + "\t" + vo.getG_jumsoo() + "\t");
		...
		lblNewLabel
				.setIcon(new ImageIcon(TodayMenu.class.getResource("/mealtime/테이블사진2.png")));
